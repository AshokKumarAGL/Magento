...
class Idev_OneStepCheckout_AjaxController extends Mage_Core_Controller_Front_Action
{
    ...
    {   
     ...
     public function newloginAction() {
        $username  = $this->getRequest()->getPost('customer_username', false);
        $password  = $this->getRequest()->getPost('customer_password', false);
        $loginUrl  = $this->getRequest()->getPost('loginurl', false);
        $session = Mage::getSingleton('customer/session');
        $result = array(
            'success' => false
        );
        if($username && $password) {
            ...
            if(!isset($result['error'])) {
                $result['success'] = true;
                if($loginUrl=='/checkout/cart/') {
                    $result['returnurl'] = '/onestepcheckout/index/address';
                }else{
                    $result['returnurl'] = $loginUrl;
                }
           }
        } else {
            $result['error'] = $this->__('Please enter email address and password.');
        }
