...
HTML;
    ...
    protected function getPluginStatusSectionHtml()
    {
        ...
        $firstSelected = ($this->getConfigForStore('clearcode_addshoppers/settings/enabled') == 1) ? "selected=\"selected\"" : "";
        $secondSelected = ($this->getConfigForStore('clearcode_addshoppers/settings/enabled') == 1) ? "" : "selected=\"selected\"";
...
HTML;
    ...
    protected function getLoginSectionHtml()
    {
        ...
        $formKey = Mage::getSingleton('core/session')->getFormKey();
        $login = $this->getConfigForStore('clearcode_addshoppers/settings/email');
        $passwrod = $this->getConfigForStore('clearcode_addshoppers/settings/password');
                    ...
                    <fieldset id="my-fieldset">
                        <table cellspacing="0" class="form-list">
                            ...
                            <tr>
                                ...
                                <td class="input-ele">
                                    <input type="password" class="input-text" name="login[password]" size="40" maxlength="128" value="{$passwrod}"/>
                                </td>
                            </tr>
                            <tr>
                                <td></td>
...
HTML;
    ...
    protected function getNewAccSectionHtml()
    {
        ...
        $formKey = Mage::getSingleton('core/session')->getFormKey();
        $email = $this->getConfigForStore('clearcode_addshoppers/settings/email');
        $pass = $this->getConfigForStore('clearcode_addshoppers/settings/password');
...
HTML;
    ...
    protected function getAccSettingsSectionHtml()
    {
        ...
        $formKey = Mage::getSingleton('core/session')->getFormKey();
        $email = $this->getConfigForStore('clearcode_addshoppers/settings/email');
        $password = $this->getConfigForStore('clearcode_addshoppers/settings/password');
        $apiKey = $this->getConfigForStore('clearcode_addshoppers/settings/account_id');
        $shopId = $this->getConfigForStore('clearcode_addshoppers/settings/shopid');
...
HTML;
    ...
    protected function getDefaultAppsSectionHtml()
    {
        ...
        $formKey = Mage::getSingleton('core/session')->getFormKey();
        $schema = ($this->getConfigForStore('clearcode_addshoppers/settings/use_schema') == '1') ? "checked" : "";
        $default = ($this->getConfigForStore('clearcode_addshoppers/settings/social') == '1') ? "checked" : "";
        $fb = ($this->getConfigForStore('clearcode_addshoppers/settings/opengraph') == '1') ? "checked" : "";
...
HTML;
    ...
    protected function _getElementHtml(Varien_Data_Form_Element_Abstract $element)
    {
        ...
        if(($this->getConfigForStore('clearcode_addshoppers/settings/enabled') == 1))
        {
            if(($this->getConfigForStore('clearcode_addshoppers/settings/active') != 1))
            {
    ...
    protected function getConfigForStore($path)
    {
        $resource = Mage::getSingleton('core/resource');
        $readConnection = $resource->getConnection('core_read');

        $storeId = Mage::app()->getStore($this->storeCode)->getId();
