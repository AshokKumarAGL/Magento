...
class Magestore_Bannerslider_Adminhtml_BannersliderController extends Mage_Adminhtml_Controller_Action
{
	...
	public function saveAction() {
		if ($data = $this->getRequest()->getPost()) {
			...
			if($data['thumbfilename']['delete']==1){
				$data['thumbfilename']='';
			}
			elseif(is_array($data['thumbfilename'])){
				$data['thumbfilename']=$data['thumbfilename']['value'];
			}
			if(isset($_FILES['filename']['name']) && $_FILES['filename']['name'] != '') {
				try {	
					...
					$data['filename'] = $_FILES['filename']['name'];
                        ...
                        if(isset($_FILES['thumbfilename']['name']) && $_FILES['thumbfilename']['name'] != '') {
				try {	
					/* Starting upload */	
					$uploader = new Varien_File_Uploader('thumbfilename');
					...
					// We set media as the upload dir
					$path = Mage::getBaseDir('media') . DS ;
					$result = $uploader->save($path, $_FILES['thumbfilename']['name'] );
					$data['thumbfilename'] = $result['file'];
				} catch (Exception $e) {
					$data['thumbfilename'] = $_FILES['thumbfilename']['name'];
		        }
			}
