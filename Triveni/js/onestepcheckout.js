...
function get_separate_save_methods_function(url, update_payments)
{
    ...
    return function(e)    {
        ...
        /* Find payment parameters and include */
	/*Include netBankingCards, card Option1, cardOption2, NonMotoCardType for avenues */
        var items = $$('input[name^=payment]',
                       'select[name^=payment]',
                       'select[name^=netBankingCards]',
                       'input[name^=cardOption1]',
                       'select[name^=NonMotoCardType]',
                       'input[name^=cardOption2]'
                      );
        var names = items.pluck('name');
        var values = items.pluck('value');
